...
This module is responsible for validating JSON files against a predefined schema
to ensure they adhere to the structure required by the Challenge 1A of the hackathon.
"""
...
class SchemaValidator:
    ...
    def __init__(self):
        """Initialize the schema validator with the official schema definition."""
        # Define the official schema for validation
    ...
    def validate_json_file(self, json_path: Path) -> Tuple[bool, List[str]]:
        ...
        This method reads a JSON file and validates its content against the predefined schema.
    ...
    def validate_data(self, data: Dict[str, Any]) -> Tuple[bool, List[str]]:
        ...
        This method performs both schema validation and additional custom validations.
        ...
        try:
            # Perform basic schema validation using jsonschema
    ...
    def _custom_validations(self, data: Dict[str, Any]) -> List[str]:
        """
        Perform additional custom validations on the data.

        This method checks for conditions not covered by the JSON schema, such as
        minimum length requirements and specific value constraints.
    ...
    def _validate_outline_item(self, item: Dict[str, Any], index: int) -> List[str]:
        """
        Validate an individual outline item.

        This method checks the level, text, and page fields of an outline item.

        Args:
            item: Dictionary containing the outline item data.
            index: Index of the item in the outline list.
    ...
    def get_validation_report(self, json_path: Path) -> str:
        """
        Generate a detailed validation report for a JSON file.

        This method validates a JSON file and generates a detailed report of the results.
        ...
        if is_valid:
            ...
            report += "   - Ensure 'title' is a non-empty string\n"
            report += "   - Ensure 'outline' is an array of heading objects\n"
            report += "   - Each heading must have 'level' (H1/H2/H3), 'text' (string), and 'page' (integer)\n"
...
def validate_output_directory(output_dir: Path) -> None:
    ...
    This function checks all JSON files in a directory for schema compliance.
